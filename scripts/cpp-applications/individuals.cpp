...
int main(int argc, char *argv[]) {
    ...
    // Read column file
    const std::filesystem::path col_file("columns.txt");
    ...
    out_name << "chr" << chr << "n-" << counter << "-" << stop;
...
//    std::string cmd = "tar -zcf ";
    ...
    compressed_name << "chr" << chr << "n-" << counter << "-" << stop; // outputfile = "chr{}n-{}-{}.tar.gz".format(c, counter, stop)
//    cmd.append(out_name.str());
//    cmd.append(".tar.gz ");
//    cmd.append(out_name.str());
    std::stringstream cmd;
    cmd << "tar -zcf " << compressed_name.str() << ".tar.gz " << compressed_name.str();
